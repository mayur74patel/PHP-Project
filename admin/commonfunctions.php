...
function addminister($minister_name,$minister_category,$newfilename) {
    global $conn;
	$insert_query = "INSERT INTO minister SET minister_name='" . $minister_name . "',minister_category='" . $minister_category . "',minister_image='" .$newfilename. "' ";
    $insert_res = mysqli_query($conn, $insert_query);
    if (mysqli_insert_id($conn)) {
        return TRUE;
	}
...
function updateminister($iCategoryID,$minister_name,$minister_category,$newfilename){
    ...
    if (isset($newfilename) && $newfilename != "") {
        ...
        if (mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_array($res);
            unlink($CATEGORY_IMAGE_PATH . $row['minister_no']);
        }
